...
import { GoogleVisionAPI } from 'react-native-google-places-autocomplete'
...
const HomeScreen = ({ navigation }) => {
  ...
  const detectObjects = async imageUri => {
    const visionAPI = new GoogleVisionAPI(
      'AIzaSyDqiXgi_9gVoWlXHE7wZDPSBE40wwPtAmw'
    )

    try {
      const result = await visionAPI.annotate({
        imageUri: imageUri,
        features: {
